...
export function analyzeMedicationOpioidSameDayConstipationBurdenCandidate(
  ...
  return analyzeMedicationSignalCandidate(features, baselines, {
    insightKey: 'medication_opioid_same_day_constipation_burden',
    subtype: 'medication_opioid_same_day_constipation_burden',
    ...
    exposureDay: (day) => hasMedicationFamily(day, 'opioid'),
    ...
    notes: [
      'Exposure days are days with opioid-family medication signals, using normalized medication matches when available and family heuristics otherwise.',
    ],
  });
}